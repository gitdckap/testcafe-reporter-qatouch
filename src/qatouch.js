...
class QaTouch {
    ...
    _validate(options, name) {
        if (options == null) {
            throw new Error("Missing Qa Touch options in .env file");
        }
        if (options[name] == null) {
            throw new Error(`Missing ${name} value. Please update Qa Touch option in .env file`);
        }
    }
    ...
    addResultsForTestRun(results) {
        ...
        this._patch(`testRunResults/status/multiple?project=${this.options.projectKey}&test_run=${this.options.testRunId}&result=${JSON.stringify(result)}&comments=Status changed by testcafe automation script.`);
    }
